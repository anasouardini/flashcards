...
let model = (0, model_1.loadList)({ item: `german-voc-new.json` });
// console.log({ model })
if (!model) {
    process.exit(2);
    ;
...
if (!model.cards) {
    ...
    model.levels.forEach((level, levelIndex) => {
        level.cards.forEach((card) => {
            ...
            const prepCard = new srs_sm2_1.SrsCardC({
...
if (model.history.levels) {
    ...
    Object.entries(model.history.lengths)
        ...
        model.history.lengths[key] = lengthsList
            ...
            if (parsedPriorDate !== parsedCurrentDate) {
                acc.push(lengthObj);
            }
            return acc;
        }, []);
